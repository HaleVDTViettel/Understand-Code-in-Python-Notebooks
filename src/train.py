...
def main():
    ...
    for fold in range(parameter.k_folds):
        if len(args.folds) > 0 and str(fold) not in args.folds.split(','):
            continue
        model = MarkdownModel((args.model_name), seq_length=parameter.seq_length, pretrained=True)
        model.zero_grad()
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch, n_accumulate=1, is_adversial=False):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        if is_adversial:
            # 对抗训练
            fgm.attack()  # Add adversarial perturbations to the embedding
            output = model(seq, seq_mask)
            loss_adv = criterion(output, target, target_mask)
            loss_adv.backward()  # Backpropagation, and accumulating the gradient of adversarial training based on normal grad
